...
class Frontend_Handler {
	...
	public function render_poster_editor_page() {
		if ( ! isset( $_GET['post_id'] ) ) {
			wp_die( __( 'ID de post no vàlid.', 'my-pdf-plugin' ) );
		}
		...
		if ( ! $post || ! current_user_can( 'edit_post', $post_id ) ) {
			wp_die( __( 'No teniu permís per editar aquest post.', 'my-pdf-plugin' ) );
		}
		...
		<div class="wrap gem-poster-studio-wrap">
			<div id="gem-poster-studio-app" class="pdf-editor-fullscreen" 
				...
				<div class="pdf-editor-header">
					...
					<div class="header-right">
						<div class="studio-badge">V1.5 Professional</div>
					</div>
				</div>
							...
							<div class="pdf-preview-page">
								...
								<div class="pdf-preview-image-container" style="height: <?php echo esc_attr($image_height); ?>mm; width: <?php echo esc_attr($image_width); ?>mm; margin: 0 auto;">
									<?php if ( $image_url ) : ?>
										...
										<div class="image-drag-handle">↕ Arrossega per enquadrar</div>
									<?php else : ?>
								...
								<div class="pdf-preview-content">
									...
									<div class="pdf-preview-columns">
										...
										<div class="pdf-col-right">
											...
											<span class="pdf-web-link">www.gem-malgrat.cat</span></p>
	...
	public function enqueue_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'               => wp_create_nonce( 'poster_studio_nonce' ),
	...
	public function handle_generate_pdf() {
		...
		error_log('PosterStudio: Received nonce: ' . $received_nonce);

		// Check nonce for security.
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'poster_studio_nonce' ) ) {
			error_log('PosterStudio: Nonce verification failed for poster_studio_nonce');
			wp_send_json_error( array( 'message' => __( 'Security check failed.', 'poster-studio' ) ) );
		}
		...
		try {
			...
			$pdf_url = $this->generate_pdf_file( $post, $custom_options );
			
			wp_send_json_success( array(
				'pdf_url' => $pdf_url,
				'message' => __( 'PDF generated successfully.', 'poster-studio' ),
			) );
		} catch ( Exception $e ) {
	 ...
	 */
	private function generate_pdf_file( $post, $options = array() ) {
		...
		$pdf_file_path = $pdf_generator->generate_from_post( $post->ID, $options );
